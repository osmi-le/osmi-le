...
- 실제로 아침에 일어나기 어려운 사람으로써 내가 원하는 기능을 직접 제안하고 만들었습니다. 여러가지 방식으로 알람을 끄고, 알람을 끈 후에 날씨나 일정 앱을 각각 실행해야 하는 번거로움이 있어서 알람이 꺼지면 날씨와 구글 캘린더에 저장된 일정 등을 확인 하고 내 기분과 오늘의 한마디 등을 입력하고 볼 수 있게 만들었습니다. phpAdmin을 이용한 mySQL 데이터베이스 연동과 php, GPS모듈, 날씨 API, 차트 라이브러리 등 다양한 기능을 구현할 수 있는 프로젝트였습니다.   
